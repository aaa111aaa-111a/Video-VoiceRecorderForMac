...
final class ContentFilterBuilderTests: XCTestCase {
    func testExcludesOwnBundleIdentifierOnly() {
        let all = ["us.zoom.xos", "app.optirecord.OptiRecord", "com.apple.finder"]
        let excluded = ContentFilterBuilder.bundleIdentifiersToExcludeFromDisplay(allBundleIdentifiers: all, ownBundleIdentifier: "app.optirecord.OptiRecord")
        XCTAssertEqual(excluded, ["app.optirecord.OptiRecord"])
    }
