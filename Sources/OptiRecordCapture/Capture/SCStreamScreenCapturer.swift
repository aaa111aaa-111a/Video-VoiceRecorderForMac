...
/// entire reason OptiRecord exists (see docs/ARCHITECTURE.md), so this type is the most
...
public final class SCStreamScreenCapturer: NSObject, ScreenCapturing {
    ...
    private let videoQueue = DispatchQueue(label: "app.optirecord.capture.screen.video")
    private let audioQueue = DispatchQueue(label: "app.optirecord.capture.screen.audio")
    private let microphoneQueue = DispatchQueue(label: "app.optirecord.capture.screen.microphone")
